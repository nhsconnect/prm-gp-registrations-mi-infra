...
def create_modify_csv(file_path: str, modify_file_path: str, headers_list: list, modify_headers: list, write_to_file: bool,
    ...
    with open(file_path, newline='') as original, open(modify_file_path, 'w', newline='') as output:
        ...
        csv_modified_rows = [{key: row[key] for key in modify_headers} for row in reader]
...
def get_gp_latest_ods_csv(service):
    ...
    create_modify_csv(epraccur_csv_file, 'initial_full_gps_ods.csv', GP_FILE_HEADERS , ['OdsCode', 'GpName', 'IcbOdsCode'], True)
...
def get_icb_latest_ods_csv(service):
    ...
    for release, url in download_url_by_release.items():
        ...
        output_name = 'update_icb_' + release + '.csv' if not is_quarterly_release else 'initial_full_icb_ods.csv'
        ...
        if epraccur_zip_file:= service.unzipping_files(download_file, zip_file_path, True):
            if epraccur_csv_file := service.unzipping_files(epraccur_zip_file, csv_file_name):
                csv_modified_rows = create_modify_csv(epraccur_csv_file, output_name, ICB_FILE_HEADERS , ['OdsCode', 'IcbName'], is_quarterly_release, icb_update_changes)
